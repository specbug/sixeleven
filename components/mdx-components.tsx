...
img: ({ src, alt }: { src?: string; alt?: string }) => {
  ...
  return (
    <figure className="my-8">
      ...
      {alt && <figcaption className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">{alt}</figcaption>}
    </figure>
  )
},
...
code: ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  if (!className) {
    return (
      <code className="bg-gray-100 dark:bg-[#171600] text-gray-900 dark:text-gray-100 rounded px-1.5 py-0.5 font-mono text-base">
        {children}
      </code>
    )
  }
  ...
  if (className === "language-pgp") {
    return (
      <code className="block p-4 rounded-lg bg-[#171600] text-white overflow-x-auto my-4 font-mono text-sm whitespace-pre">
        {children}
      </code>
    )
  }
  ...
  return (
    <code
      className={`block p-4 rounded-lg bg-gray-100 dark:bg-[#171600] text-gray-900 dark:text-gray-100 overflow-x-auto my-6 font-mono ${className}`}
    >
      {children}
    </code>
  )
